...
if [ -f "$Dir/deb_and_ppa_templates/copyright" ]; then
    cp $Dir/deb_and_ppa_templates/copyright $Dir/deb_and_ppa_beta/cyberfox-$VERSION/debian
else
...
if [ -d "../../../../../obj64/dist/Cyberfox" ]; then
	...
	cp $Dir/deb_and_ppa_templates/vendor-gre.js $Dir/deb_and_ppa_beta/cyberfox-$VERSION
else
...
chmod  755 $Dir/deb_and_ppa_beta/cyberfox-$VERSION/debian/copyright
...
# Clean up
if [ -f "$Dir/deb_and_ppa_beta" ]; then
    echo "Clean: $Dir/deb_and_ppa_beta"
    rm -rf $Dir/deb_and_ppa_beta
fi
