             ...
             LPCWSTR aClassName)
{
  ...
  LPCWSTR progID = isProtocol ? L"CyberfoxURL" : L"CyberfoxHTML";
                     ...
                     LPCWSTR aClassName)
{
  ...
  // We can save the value only if Cyberfox is the default.
  if (!isDefault) {
    return isDefault;
  }
                        ...
                        LPCWSTR aClassName)
{
  ...
  bool isProtocol = *aClassName != L'.';
  nsString progId = isProtocol ? NS_LITERAL_STRING("CyberfoxURL")
                               : NS_LITERAL_STRING("CyberfoxHTML");
...
nsWindowsShellService::InvokeHTTPOpenAsVerb()
{
  ...
  nsresult rv = formatter->FormatURLPref(
    NS_LITERAL_STRING("app.helpdoc.baseURL"), urlStr);
  if (NS_FAILED(rv)) {
    return rv;
  }
